...
with patch('fortishield.common.fortishield_uid'):
    with patch('fortishield.common.fortishield_gid'):
        ...
        from fortishield.core.tests.test_rootcheck import InitRootcheck, send_msg_to_fdb, remove_db, \
...
def test_rootcheck_run(close_mock, send_mock, connect_mock, agent_init__mock, agent_exit__mock,
    ...
    agent_list : list
        List of agent IDs.
    failed_items : list
        List with the FortishieldDBQueryAgents response.
    status_list : list
    ...
    with patch('fortishield.rootcheck.FortishieldDBQueryAgents.run', return_value=failed_items[0]):
        ...
        assert isinstance(result, rootcheck.AffectedItemsFortishieldResult)
...
@pytest.mark.parametrize('agent_list, expected_affected_items, expected_calls, fdb_side_effect', [
    (['002'], [], [call('agent 002 rootcheck delete')], FortishieldError(2004)),
...
def test_clear(mock_connect, mock_info, agent_list, expected_affected_items, expected_calls, fdb_side_effect):
    ...
    expected_calls : list
        List of expected calls to the mocked FortishieldDBConnection._send function.
    fdb_side_effect : FortishieldError or list
        Side effect used in the mocked FortishieldDBConnection._send function.
    """
    with patch('fortishield.core.fdb.FortishieldDBConnection._send', side_effect=fdb_side_effect) as mock_fdbconn:
        ...
        mock_fdbconn.assert_has_calls(expected_calls, any_order=True)
...
def test_get_rootcheck_agent(mock_connect, mock_send, mock_info, mock_exists, limit):
    ...
    for item in result['affected_items']:
        for key in item.keys():
            assert key in FortishieldDBQueryRootcheck.fields
...
def test_get_rootcheck_agent_select(mock_connect, mock_send, mock_info, mock_exists, select):
    ...
    result = rootcheck.get_rootcheck_agent(agent_list=['001'], select=select, filters={'status': 'all'}).render()[
        'data']
    select = select if select else list(FortishieldDBQueryRootcheck.fields.keys())

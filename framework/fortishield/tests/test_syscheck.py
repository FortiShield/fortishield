...
@patch('fortishield.core.fdb.FortishieldDBConnection.__init__', return_value=None)
@patch('fortishield.core.fdb.FortishieldDBConnection.execute', return_value=None)
@patch('fortishield.core.fdb.FortishieldDBConnection.close')
def test_syscheck_clear(fdb_close_mock, fdb_execute_mock, fdb_init_mock, agent_list, expected_result, agent_info_list,
                        agent_version):
    ...
    with patch('fortishield.syscheck.get_agents_info', return_value=set(agent_info_list)), \
            patch('fortishield.syscheck.FortishieldDBQueryAgents') as mock_fdbqa:
        mock_fdbqa.return_value.run.return_value = {
...
@patch('fortishield.core.fdb.FortishieldDBConnection.__init__', return_value=None)
@patch('fortishield.core.fdb.FortishieldDBConnection.execute', side_effect=FortishieldError(1000))
@patch('fortishield.core.fdb.FortishieldDBConnection.close')
def test_syscheck_clear_exception(fdb_close_mock, execute_mock, fdb_init_mock, agent_list, expected_result,
...
def test_syscheck_last_scan(socket_mock, fdb_conn_mock, db_mock, exists_mock, agent_id, fortishield_version):
...
def test_syscheck_files(socket_mock, exists_mock, agent_id, select, filters, distinct, q):
    ...
    with patch('fortishield.core.utils.FortishieldDBConnection') as mock_fdb:
        mock_fdb.return_value = InitWDBSocketMock(sql_schema_file='schema_syscheck_test.sql')

...
DEFAULT_CTI_URL = 'https://cti-fortishield.khulnasoft.com'
...
def _rcl2json(filepath: str) -> dict:
    ...
    try:
        ...
        raise FortishieldError(1101, str(e))
...
def get_ossec_conf(section: str = None, field: str = None, conf_file: str = common.OSSEC_CONF,
    ...
    FortishieldError(1101)
        Requested component does not exist.
    FortishieldError(1102)
        Invalid section.
    FortishieldError(1103)
        Invalid field in section.
    FortishieldError(1106)
    ...
    if section:
        try:
            ...
            if section not in CONF_SECTIONS.keys():
                raise FortishieldError(1102, extra_message=e.args[0])
            else:
                raise FortishieldError(1106, extra_message=e.args[0])
...
def get_agent_conf(group_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if not os_path.exists(os_path.join(common.SHARED_PATH, group_id)):
        raise FortishieldResourceNotFound(1710, group_id)
    ...
    if not os_path.exists(agent_conf):
        raise FortishieldError(1006, agent_conf)
...
def get_agent_conf_multigroup(multigroup_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if multigroup_id and not os_path.exists(os_path.join(common.MULTI_GROUPS_PATH, multigroup_id)) or not multigroup_id:
        raise FortishieldResourceNotFound(1710, extra_message=multigroup_id if multigroup_id else "No multigroup provided")
    ...
    if not os_path.exists(agent_conf):
        raise FortishieldError(1006, extra_message=os_path.join("FORTISHIELD_PATH", "var", "multigroups", agent_conf))
...
def get_file_conf(filename: str, group_id: str = None, type_conf: str = None, raw: bool = False) -> dict | str:
    ...
    if not os_path.exists(file_path):
        raise FortishieldError(1006, file_path)
    ...
    if type_conf:
        if type_conf in types:
            ...
            raise FortishieldError(1104, f'{type_conf}. Valid types: {types.keys()}')
    else:
...
def parse_internal_options(high_name: str, low_name: str) -> str:
    ...
    Raises
    ------
    FortishieldInternalError(1107)
        Internal options file not found.
    FortishieldInternalError(1108)
    ...
    if not os_path.exists(common.INTERNAL_OPTIONS_CONF):
        raise FortishieldInternalError(1107)
    ...
    try:
        ...
        raise FortishieldInternalError(1108, e.args[0])
...
def get_internal_options_value(high_name: str, low_name: str, max_: int, min_: int) -> int:
    ...
    Raises
    ------
    FortishieldError(1109)
        Option must be a digit.
    FortishieldError(1110)
    ...
    if not option.isdigit():
        raise FortishieldError(1109, f'Option: {high_name}.{low_name}. Value: {option}')

    option = int(option)
    if option < min_ or option > max_:
        raise FortishieldError(1110, f'Max value: {max_}. Min value: {min_}. Found: {option}.')
...
def upload_group_configuration(group_id: str, file_content: str) -> str:
    ...
    FortishieldError(1113)
        XML syntax error.
    FortishieldError(1114)
        Fortishield syntax error.
    FortishieldError(1115)
        Error executing verify-agent-conf.
    FortishieldInternalError(1743)
        Error running Fortishield syntax validator.
    ...
    try:
        # check Fortishield xml format
        try:
            subprocess.check_output([os_path.join(common.FORTISHIELD_PATH, "bin", "verify-agent-conf"), '-f', tmp_file_path],
            ...
            if output_regex:
                raise FortishieldError(1114, ' '.join(output_regex))
            else:
                raise FortishieldError(1115, e.output.decode())
        except Exception as e:
            raise FortishieldInternalError(1743, str(e))
...
def upload_group_file(group_id: str, file_data: str, file_name: str = 'agent.conf') -> str:
    ...
    FortishieldResourceNotFound(1710)
        Group was not found.
    FortishieldError(1112)
        Empty files are not supported.
    FortishieldError(1111)
...
def get_active_configuration(agent_id: str, component: str, configuration: str) -> dict:
    ...
    component_socket_mapping = {'agent': 'analysis', 'agentless': 'agentless', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'fortishield-db': 'fdb', 'wmodules': 'wmodules'}
    ...
    if not component or not configuration:
        raise FortishieldError(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise FortishieldError(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = os_path.join(common.FORTISHIELD_PATH, "queue", component_socket_dir_mapping[component],
                                   component_socket_mapping[component])

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise FortishieldError(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            try:
                ...
                raise FortishieldInternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise FortishieldInternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.FORTISHIELD_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise FortishieldError(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,

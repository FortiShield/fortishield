...
def get_FortishieldDBQuery_params(fdb_class):
    """Get default parameters for the specified FortishieldDBQuery class.

    Parameters
    ----------
    fdb_class : str
        Suffix of the FortishieldDBQuery class. Example: 'Agents' to get default parameters from `FortishieldDBQueryAgents` class.
    ...
    """
    with patch('fortishield.core.agent.FortishieldDBQuery.__init__') as fdbquery_mock:
        getattr(sys.modules[__name__], f'FortishieldDBQuery{fdb_class}')()
        return fdbquery_mock.call_args.kwargs


# list with Fortishield packages availables with their hash
...
def test_FortishieldDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    ...
    if value:
        FortishieldDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(FortishieldException, match=".* 2005 .*"):
            FortishieldDBQueryAgents()


@patch('socket.socket.connect')
def test_FortishieldDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of FortishieldDBQueryAgents returns expected query"""
    query_agent = FortishieldDBQueryAgents()
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of FortishieldDBQueryAgents returns expected result
    ...
    """
    query_agent = FortishieldDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of FortishieldDBQueryAgents returns expected query"""
    query_agent = FortishieldDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of FortishieldDBQueryAgents returns expected data"""
    query_agent = FortishieldDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of FortishieldDBQueryAgents returns expected query"""
    query_agent = FortishieldDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of FortishieldDBQueryAgents returns expected query
...
def test_FortishieldDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of FortishieldDBQueryGroup works properly.
        ...
        """
    query_group = FortishieldDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of FortishieldDBQueryGroupByAgents works properly."""
    query_group = FortishieldDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of FortishieldDBQueryGroupByAgents works properly."""
    query_group = FortishieldDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)
...
def test_FortishieldDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    ...
    query_group = FortishieldDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
def test_FortishieldDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    ...
    query_group = FortishieldDBQueryGroup()
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of FortishieldDBQueryMultigroups works properly."""
    query_multigroups = FortishieldDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of FortishieldDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = FortishieldDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_FortishieldDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of FortishieldDBQueryMultigroups works properly."""
    query_multigroups = FortishieldDBQueryMultigroups(group_id='test')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(FortishieldResourceNotFound, match='.* 1701 .*'):
...
def test_agent_get_key_ko(socket_mock, send_mock):
    ...
    with pytest.raises(FortishieldError, match='.* 1703 .*'):
        agent = Agent('000')
        agent.get_key()
...
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(FortishieldQueue(common.AR_SOCKET))
...
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(FortishieldError, match='.* 1707 .*'):
        agent = Agent('003')
...
def test_agent_add_ko(mock_maganer_status):
    ...
    with pytest.raises(FortishieldError, match='.* 1706 .*'):
...
@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (FortishieldError(9008, cmd_error=True), ".* 1705 .*"),
    (FortishieldError(9007, cmd_error=True), ".* 1706 .*"),
...
def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with pytest.raises(FortishieldError, match='.* 1752 .*'):
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(FortishieldError, match='.* 1722 .*'):
...
def test_agent_get_agent_groups(socket_connect_mock, send_mock):
    ...
    fdb_command = 'global select-group-belong :agent_id:'
    send_mock.assert_called_once_with(fdb_command.replace(':agent_id:', agent_id), raw=True)
...
def test_agent_set_agent_group_relationship(socket_connect_mock, send_mock, remove, override, expected_mode):
    ...
    expected_mode: str
        Expected mode to send to fdb to change the relationship between an agent and a group.
    """
    agent_id = '001'
    group_id = 'default'
    fdb_command = r'global set-agent-groups {\"mode\":\"(.+)\",\"sync_status\":\"syncreq\",\"data\":\[{\"id\":(.+),' \
                  r'\"groups\":\[\"(.+)\"]}]}'

    # Default relationship -> add an agent to a group
    Agent.set_agent_group_relationship(agent_id, group_id, remove, override)
    match = re.match(fdb_command, send_mock.call_args[0][0])
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(FortishieldInternalError, match='.* 2005 .*'):
...
def test_agent_get_config_ko(socket_mock, send_mock, mock_fortishield_socket):
    ...
    with pytest.raises(FortishieldError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'Fortishield v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(FortishieldError, match=".* 1307 .*"):
        agent.get_config('com', None, 'Fortishield v4.0.0')
        agent.get_config(None, 'active-response', 'Fortishield v4.0.0')

    # Agent Fortishield version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(FortishieldInternalError, match=".* 1735 .*"):
...
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    with patch('fortishield.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            ...
            expected_msg = FortishieldQueue.RESTART_AGENTS_JSON if FortishieldVersion(
                agent_version['version']) >= FortishieldVersion(common.AR_LEGACY_VERSION) else FortishieldQueue.RESTART_AGENTS
...
@pytest.mark.parametrize('group, fdb_response, expected_agents', [
...
@patch('socket.socket.connect')
def test_expand_group(socket_mock, group, fdb_response, expected_agents):
    ...
    fdb_response: list
        Mock return values for the `FortishieldDBConnection.send` method.
    expected_agents : set
    ...
    with patch('fortishield.core.fdb.FortishieldDBConnection.send', side_effect=fdb_response):

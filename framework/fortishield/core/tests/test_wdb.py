# Copyright (C) 2015, KhulnaSoft Ltd.
# Created by KhulnaSoft, Ltd. <info@khulnasoft.com>.
# This program is free software; you can redistribute it and/or modify it under the terms of GPLv2

import asyncio  # noqa
import struct
from unittest.mock import patch, AsyncMock, MagicMock, call

import pytest

from fortishield.core import common
from fortishield.core import exception
from fortishield.core.common import MAX_SOCKET_BUFFER_SIZE
from fortishield.core.fdb import AsyncFortishieldDBConnection, FortishieldDBConnection


def format_msg(msg):
    return struct.pack('<I', len(bytes(msg)))


def test_async_init():
    """Verify that AsyncFortishieldDBConnection attributes are correct."""
    async_fdb = AsyncFortishieldDBConnection('test')
    assert async_fdb.socket_path == common.WDB_SOCKET
    assert async_fdb.loop == 'test'
    assert async_fdb._reader is None
    assert async_fdb._writer is None


@pytest.mark.asyncio
@patch('asyncio.open_unix_connection', return_value=[AsyncMock(), MagicMock()])
async def test_async_open_connection(open_unix_connection_mock):
    """Verify that open_unix_connection is called with expected parameters."""
    async_fdb = AsyncFortishieldDBConnection(loop='test_loop')
    await async_fdb.open_connection()
    assert async_fdb._reader is not None
    assert async_fdb._writer is not None
    open_unix_connection_mock.assert_awaited_once_with(path=common.WDB_SOCKET)


def test_async_close():
    """Check whether stream close method is called."""
    async_fdb = AsyncFortishieldDBConnection()
    async_fdb._writer = MagicMock()
    async_fdb.close()
    async_fdb._writer.close.assert_called_once_with()


@pytest.mark.asyncio
@pytest.mark.parametrize('raw, expected_response', [
    (False, {'test': 'test response'}),
    (True, ['ok', '{"test": "test response"}'])
])
async def test_async_send(raw, expected_response):
    """Assert that expected response is returned and methods are called with expected parameters."""
    msg = 'test message'
    encoded_response = 'ok {"test": "test response"}'.encode(encoding='utf-8')
    async_fdb = AsyncFortishieldDBConnection()
    async_fdb._reader = AsyncMock()
    async_fdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    async_fdb._writer = MagicMock()
    async_fdb._writer.drain = AsyncMock()

    result = await async_fdb._send(msg, raw=raw)

    assert result == expected_response
    async_fdb._writer.write.assert_called_once_with(
        struct.pack('<I', len(msg.encode(encoding='utf-8'))) + msg.encode(encoding='utf-8'))
    async_fdb._writer.drain.assert_called_once_with()
    async_fdb._reader.readexactly.assert_has_calls([call(4), call(28)])


@pytest.mark.asyncio
async def test_async_send_ko():
    """Verify that expected exception codes are raised."""
    async_fdb = AsyncFortishieldDBConnection()

    # Reader and writer are None.
    with pytest.raises(exception.FortishieldInternalError, match=".* 2005 .*"):
        await async_fdb._send('test')

    # EOF reached before n can be read.
    async_fdb._writer = MagicMock()
    async_fdb._writer.drain = AsyncMock()
    async_fdb._reader = AsyncMock()
    async_fdb._reader.readexactly.side_effect = lambda x: exec('raise(asyncio.IncompleteReadError("test", 5))')
    with pytest.raises(exception.FortishieldInternalError, match=r'\b2010\b'):
        await async_fdb._send('test')

    # Fortishield-db error response.
    encoded_response = 'err Error message'.encode(encoding='utf-8')
    async_fdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    with pytest.raises(exception.FortishieldError, match=r'\b2003\b'):
        await async_fdb._send('test')


@pytest.mark.asyncio
async def test_run_fdb_command():
    """Test `FortishieldDBConnection.run_fdb_command` method."""
    send_result = ('status', '["data"]')
    command = "any fdb command"

    fdb_con = AsyncFortishieldDBConnection()
    with patch('fortishield.core.fdb.AsyncFortishieldDBConnection._send', return_value=send_result) as fdb_send_mock:
        result = await fdb_con.run_fdb_command(command)
        fdb_send_mock.assert_called_once_with(command, raw=True)

    assert result == send_result, 'Expected command response does not match'


@pytest.mark.asyncio
@pytest.mark.parametrize('fdb_response', [
    ('err', 'Extra custom test message'),
    ('err', )
])
async def test_run_fdb_command_ko(fdb_response):
    """Test `FortishieldDBConnection.run_fdb_command` method expected exceptions."""
    with patch('fortishield.core.fdb.AsyncFortishieldDBConnection._send', return_value=fdb_response):
        fdb_con = AsyncFortishieldDBConnection()
        with pytest.raises(exception.FortishieldInternalError, match=".* 2007 .*") as expected_exc:
            await fdb_con.run_fdb_command("global sync-agent-info-get ")

        if len(fdb_response) > 1:
            assert fdb_response[1] in expected_exc.value.message, 'Extra message was not added to exception'


def test_failed_connection():
    """
    Tests an exception is properly raised when it's not possible to connect to fdb
    """
    # tests the socket path doesn't exists
    with patch('fortishield.core.common.WDB_SOCKET', '/this/path/doesnt/exist'):
        with pytest.raises(exception.FortishieldException, match=".* 2005 .*"):
            FortishieldDBConnection()
    # tests an exception is properly raised when a connection error is raised
    with patch('socket.socket') as socket_patch:
        with pytest.raises(exception.FortishieldException, match=".* 2005 .*"):
            socket_patch.return_value.connect.side_effect = ConnectionError
            FortishieldDBConnection()


@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_wrong_character_encodings_fdb(send_mock, connect_mock):
    """
    Tests receiving a text with a bad character encoding from fortishield db
    """
    def recv_mock(size_to_receive):
        bad_string = b' {"bad": "\x96bad"}'
        return format_msg(bad_string) if size_to_receive == 4 else bad_string

    with patch('socket.socket.recv', side_effect=recv_mock):
        myfdb = FortishieldDBConnection()
        received = myfdb._send("test")
        assert received == {"bad": "bad"}


@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_null_values_are_removed(send_mock, connect_mock):
    """
    Tests '(null)' values are removed from the resulting dictionary
    """
    def recv_mock(size_to_receive):
        nulls_string = b' [{"a": "a", "b": "(null)", "c": [1, 2, 3], "d": {"e": "(null)"}}]'
        return format_msg(nulls_string) if size_to_receive == 4 else nulls_string

    with patch('socket.socket.recv', side_effect=recv_mock):
        myfdb = FortishieldDBConnection()
        received = myfdb._send("test")
        assert received == [{"a": "a", "c": [1, 2, 3], "d": {}}]


@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_failed_send_private(send_mock, connect_mock):
    """
        Tests an exception is properly raised when it's not possible to send a msg to the fdb socket
    """
    def recv_mock(size_to_receive):
        error_string = b'err {"agents": {"001": "Error"}}'
        return format_msg(error_string) if size_to_receive == 4 else error_string

    with patch('socket.socket.recv', side_effect=recv_mock):
        myfdb = FortishieldDBConnection()
        with pytest.raises(exception.FortishieldException, match=".* 2003 .*"):
            myfdb._send('test_msg')

    with patch('socket.socket.recv', return_value=b'a' * (MAX_SOCKET_BUFFER_SIZE + 1)):
        myfdb = FortishieldDBConnection()
        with pytest.raises(exception.FortishieldException, match=".* 2009 .*"):
            myfdb._send('test_msg')


@pytest.mark.parametrize('content', [
    b'ok {"agents": {"001": "Ok"}}',
    b'ok {"agents": {"0ad": "Invalid agent ID"}}',
    b'ok {"agents": {"001": "DB waiting for deletion"}}',
    b'ok {"agents": {"001": "DB not found"}}'
])
@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_remove_agents_database(send_mock, connect_mock, content):
    """
    Tests delete_agents_db method handle exceptions properly
    """
    def recv_mock(size_to_receive):
        return format_msg(content) if size_to_receive == 4 else content

    with patch('socket.socket.recv', side_effect=recv_mock):
        myfdb = FortishieldDBConnection()
        received = myfdb.delete_agents_db(['001', '002'])
        assert(isinstance(received, dict))
        assert("agents" in received)


@pytest.mark.parametrize('error_query', [
    'Agent sql select test',
    'error sql select test',
    'agent bad_digit sql select test',
    'agent 000 sql sql_sentence',
    'global sql delete test ;'
])
@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_query_input_validation_private(send_mock, connect_mock, error_query):
    myfdb = FortishieldDBConnection()
    with pytest.raises(exception.FortishieldException, match=".* 2004 .*"):
        myfdb.execute(error_query)


@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_query_lower_private(send_mock, connect_mock):
    myfdb = FortishieldDBConnection()
    with pytest.raises(exception.FortishieldException, match=".* 2004 .*"):
        myfdb.execute("Agent sql select 'test'")


@patch("socket.socket.connect")
@patch("socket.socket.send")
@patch("fortishield.core.fdb.FortishieldDBConnection._send")
def test_execute(send_mock, socket_send_mock, connect_mock):
    def send_mock(obj, msg, raw=False):
        return ['ok', '{"total": 5}'] if raw else [{"total": 5}]

    myfdb = FortishieldDBConnection()
    myfdb.execute('agent 000 sql delete from test', delete=True)
    myfdb.execute("agent 000 sql update test set value = 'test' where key = 'test'", update=True)
    with patch("fortishield.core.fdb.FortishieldDBConnection._send", new=send_mock):
        myfdb.execute("agent 000 sql select test from test offset 1 limit 1")
        myfdb.execute("agent 000 sql select test from test offset 1 limit 1", count=True)
        myfdb.execute("agent 000 sql select test from test offset 1 count")


@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_execute_pagination(socket_send_mock, connect_mock):
    myfdb = FortishieldDBConnection()

    # Test pagination
    with patch("fortishield.core.fdb.FortishieldDBConnection._send",
               side_effect=[[{'total': 5}], exception.FortishieldInternalError(2009), ['ok', '{"total": 5}'],
                            ['ok', '{"total": 5}']]):
        myfdb.execute("agent 000 sql select test from test offset 1 limit 500")

    # Test pagination error
    with patch("fortishield.core.fdb.FortishieldDBConnection._send",
               side_effect=[[{'total': 5}], exception.FortishieldInternalError(2009)]):
        with pytest.raises(exception.FortishieldInternalError, match=".* 2009 .*"):
            myfdb.execute("agent 000 sql select test from test offset 1 limit 1")


@pytest.mark.parametrize('error_query, error_type, expected_exception, delete, update', [
    ('agent 000 sql delete test', None, 2004, True, False),
    ('agent 000 sql update test', None, 2004, False, True),
    ('agent 000 sql select test from test offset 1 limit 1', ValueError, 2006, False, False),
    ('agent 000 sql select test from test offset 1 limit 1', Exception, 2007, False, False)
])
@patch("socket.socket.connect")
@patch("socket.socket.send")
def test_failed_execute(send_mock, connect_mock, error_query, error_type, expected_exception, delete, update):
    myfdb = FortishieldDBConnection()
    if not error_type:
        with pytest.raises(exception.FortishieldException, match=f'.* {expected_exception} .*'):
            myfdb.execute(error_query, delete=delete, update=update)
    else:
        with patch("fortishield.core.fdb.FortishieldDBConnection._send", return_value=[{'total': 5}]):
            with patch("fortishield.core.fdb.min", side_effect=error_type):
                with pytest.raises(exception.FortishieldException, match=f'.* {expected_exception} .*'):
                    myfdb.execute(error_query, delete=delete, update=update)


@pytest.mark.parametrize('string', [
    '[{"key1": "value1"}]',
    '[{"key1": "value1"}, {"invalid": "(null)"}]',
])
def test_FortishieldDBConnection_loads(string):
    """Test that the `loads` method from the class `FortishieldDBConnection` cleans empty objects from the result."""
    result = FortishieldDBConnection.loads(string)
    assert len(result) == 1
    assert result[0] == {"key1": "value1"}

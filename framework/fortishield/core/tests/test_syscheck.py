...
def test_fortishield_db_query_syscheck__init__(mock_fdbquery, mock_backend, agent):
    ...
    agent: str
        The agent to test.
    """
    syscheck.FortishieldDBQuerySyscheck(agent)
    mock_backend.assert_called_with(agent)
    mock_fdbquery.assert_called_with(backend=ANY, default_sort_field='mtime', min_select_fields=set(), count=True,
...
@patch('fortishield.core.fdb.FortishieldDBConnection')
def test_syscheck_delete_agent(mock_db_conn, agent):
    """Test if proper parameters are being sent to the fdb socket.

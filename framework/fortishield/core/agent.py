...
from fortishield.core import common, configuration, stats
from fortishield.core.InputValidator import InputValidator
from fortishield.core.cluster.utils import get_manager_status
from fortishield.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from fortishield.core.exception import FortishieldException, FortishieldError, FortishieldInternalError, FortishieldResourceNotFound
from fortishield.core.utils import FortishieldVersion, plain_dict_to_nested_dict, get_fields_to_nest, FortishieldDBQuery, \
    FortishieldDBQueryDistinct, FortishieldDBQueryGroupBy, FortishieldDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from fortishield.core.fortishield_queue import FortishieldQueue
from fortishield.core.fortishield_socket import FortishieldSocket, FortishieldSocketJSON, create_fortishield_socket_message
...
class FortishieldDBQueryAgents(FortishieldDBQuery):
    """Class used to query Fortishield agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_fortishield_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = FortishieldDBBackend(query_format='global')
        FortishieldDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Fortishield query."""
        FortishieldDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return FortishieldDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the Fortishield query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            FortishieldDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise FortishieldError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            FortishieldDBQuery._process_filter(self, field_name, field_filter, q_filter)


class FortishieldDBQueryGroup(FortishieldDBQuery):
    """Class used to query Fortishield groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = FortishieldDBBackend(query_format='global')
        FortishieldDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class FortishieldDBQueryDistinctAgents(FortishieldDBQueryDistinct, FortishieldDBQueryAgents):
    pass


class FortishieldDBQueryGroupByAgents(FortishieldDBQueryGroupBy, FortishieldDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        FortishieldDBQueryAgents.__init__(self, *args, **kwargs)
        FortishieldDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return FortishieldDBQuery._format_data_into_dictionary(self)


class FortishieldDBQueryMultigroups(FortishieldDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        FortishieldDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        FortishieldResourceNotFound(1701)
            Agent does not exist.
        """
        with FortishieldDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
    ...
    def reconnect(self, wq: FortishieldQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : FortishieldQueue
            FortishieldQueue used for the active response message.

        Raises
        ------
        FortishieldError(1707)
            If the agent to be reconnected is not active.

        Returns
        -------
        str
            Message generated by Fortishield.
        """
        # Check if agent is active
        self.get_basic_information()
        if self.status.lower() != 'active':
            raise FortishieldError(1707)

        # Send force reconnect message to the FortishieldQueue
        ret_msg = wq.send_msg_to_agent(FortishieldQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        Raises
        ------
        FortishieldError(1726)
            Authd is not running.
        FortishieldInternalError(1757)
            Unhandled exception.
        ...
        str
            Message generated by Fortishield.
        """
        # Check that fortishield-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except FortishieldInternalError as e:
            # fortishield-authd is not running due to a problem with /proc availability
            raise FortishieldError(1726, extra_message=str(e))

        if manager_status.get('fortishield-authd') != 'running':
            # fortishield-authd is not running
            raise FortishieldError(1726)
        ...
        try:
            ...
            raise FortishieldInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = FortishieldSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise FortishieldError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise FortishieldInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Fortishield using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise FortishieldError(1705, extra_message=name)
            elif e.code == 9007:
                raise FortishieldError(1706, extra_message=ip)
            elif e.code == 9012:
                raise FortishieldError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = FortishieldDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = FortishieldDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    def add_group_to_agent(group_id: str, agent_id: str, replace: bool = False, replace_list: list = None) -> str:
        ...
        FortishieldInternalError(2007)
            Body is empty.
        FortishieldError(1752)
            Could not force single group for the agent.
        FortishieldError(1737)
    ...
    def get_agent_groups(agent_id: str) -> list:
        ...
        list
            List of group IDs.
        """
        fdb = FortishieldDBConnection()
        try:
            _, payload = fdb.send(f'global select-group-belong {agent_id}', raw=True)
    ...
    def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        FortishieldResourceNotFound(1710)
            The group was not found.
        FortishieldError(1734)
            Error removing agent from group.
        FortishieldError(1745)
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise FortishieldResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise FortishieldError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise FortishieldError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Fortishield vX.Y.Z.

        Raises
        ------
        FortishieldError(1735)
        ...
        """
        if FortishieldVersion(agent_version) < FortishieldVersion(common.ACTIVE_CONFIG_VERSION):
            raise FortishieldInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        FortishieldInternalError(1015)
            Agent version is null.
        FortishieldInternalError(1735)
        ...
        if self.version is None:
            raise FortishieldInternalError(1015)
        agent_version = FortishieldVersion(self.version.split(" ")[1])
        required_version = FortishieldVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise FortishieldInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_fortishield_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_fortishield_version_format(filters: dict):
    """Verify and format the specified fortishield version into the 'fortishield vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"fortishield {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^fortishield \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: FortishieldQueue = None) -> str:
    ...
    wq : FortishieldQueue
        FortishieldQueue used for the active response messages.
    ...
    str
        Message generated by Fortishield.
    """
    # If the Fortishield agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if FortishieldVersion(agent_version) >= FortishieldVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(FortishieldQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(FortishieldQueue.RESTART_AGENTS, agent_id)
...
def expand_group(group_name: str) -> set:
    ...
    try:
        ...
        while True:
            ...
            status, payload = fdb_conn.send(command, raw=True)
...
def get_manager_name() -> str:
    ...
    """
    fdb_conn = FortishieldDBConnection()
    manager_name = fdb_conn.execute("global sql SELECT name FROM agent WHERE (id = 0)")[0]['name']
    fdb_conn.close()
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_fortishield_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_fortishield_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
                                          'wpk_repo': wpk_repo,
                                          'file_path': file_path,
    ...
    s = FortishieldSocket(common.UPGRADE_SOCKET)

...
class AsyncFortishieldDBConnection:
    """
    Represent an async connection to the fdb socket.
    """
    ...
    async def _send(self, msg, raw=False):
        ...
        try:
            ...
            try:
                ...
                raise FortishieldInternalError(2010, extra_message=e)

            if raw:
                return data
            elif data[0] == "err":
                raise FortishieldError(2003, data[1])
            else:
                return json.loads(data[1], object_hook=FortishieldDBConnection.json_decoder)
        except (FileNotFoundError, ConnectionError) as e:
            with contextlib.suppress(Exception):
                await self.open_connection()
            raise FortishieldInternalError(2005, extra_message=e)

    async def run_fdb_command(self, command):
        """Run command in fdb and return list of retrieved information.

        The response of fdb socket can contain 2 elements, a STATUS and a PAYLOAD.
        State value can be:
        ...
        if len(result) > 1:
            if result[0] == 'err':
                raise FortishieldInternalError(2007, extra_message=result[1])

        else:
            if result[0] != 'ok':
                raise FortishieldInternalError(2007)

        return result


class FortishieldDBConnection:
    """
    Represent a connection to the fdb socket.
    """
    ...
    def __init__(self, request_slice=500):
        ...
        try:
            ...
            raise FortishieldInternalError(2005, e)
    ...
    def __query_input_validation(self, query: str):
        ...
        for check, error_text in input_val_errors:
            if not check:
                raise FortishieldError(2004, error_text)

    def _send(self, msg: str, raw: bool = False) -> dict:
        """Send a message to the fdb socket.
        ...
        if data_size >= MAX_SOCKET_BUFFER_SIZE:
            raise FortishieldInternalError(2009)
        ...
        if data[0] == "err":
            ...
            return FortishieldDBConnection.loads(data[1])
    ...
    def loads(string: str) -> dict:
        ...
        dict
            JSON object.
        """
        data = json.loads(string, object_hook=FortishieldDBConnection.json_decoder)
        if '"(null)"' in string:
    ...
    def execute(self, query, count=False, delete=False, update=False):
        """
        Send a SQL query to fdb socket.
        """

        def send_request_to_fdb(query_lower, step, off, response):
            try:
                request = query_lower.replace(':limit', 'limit {}'.format(step)).replace(':offset',
                                                                                         'offset {}'.format(off))
                request_response = self._send(request, raw=True)[1]
                response.extend(FortishieldDBConnection.loads(request_response))
                ...
                send_request_to_fdb(query_lower, step // 2, off, response)
                # Add step // 2 remaining when the step is odd to avoid losing information
                return send_request_to_fdb(query_lower, step // 2 + step % 2, step // 2 + off, response)
        ...
        if delete:
            ...
            if regex.match(query_lower) is None:
                raise FortishieldError(2004, "Delete query is wrong")
        ...
        if update:
            regex = re.compile(r"\w+ \d+? sql update ([\w\d,*_ ]+) set value = '([\w\d,*_ ]+)' where key (=|like)?"
                               r" '([a-z0-9,*_%\- ]+)'")
            if regex.match(query_lower) is None:
                raise FortishieldError(2004, "Update query is wrong")
            return self._send(query_lower)
        ...
        if not re.search(r'.?select count\([\w \*]+\)( as [^,]+)? from', query_without_where):
            ...
            try:
                ...
                while off < limit + offset:
                    ...
                    self.request_slice = send_request_to_fdb(query_lower, min(limit + offset - off, step), off,
                                                             response)
                    off += step
            except ValueError as e:
                raise FortishieldError(2006, str(e))
            except (FortishieldError, FortishieldInternalError) as e:
                raise e
            except Exception as e:
                raise FortishieldInternalError(2007, str(e))

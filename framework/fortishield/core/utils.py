    ...
    for pid_file in os.listdir(common.OSSEC_PIDFILE_PATH):
        if match := re.match(regex, pid_file):
            try:
                ...
                print(f'{daemon}: Non existent process {match.group(1)}, removing from {common.FORTISHIELD_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a Fortishield framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    Raises
    ------
    FortishieldError(1400)
        Invalid offset.
    FortishieldError(1401)
        Invalid limit.
    FortishieldError(1405)
        Limit exceeding the maximum permitted.
    FortishieldError(1406)
        Invalid limit (0).
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise FortishieldError(1405, extra_message=str(limit))
        elif limit == 0:
            raise FortishieldError(1406)
    ...
    if offset < 0:
        raise FortishieldError(1400)
    elif limit < 1:
        raise FortishieldError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    Raises
    ------
    FortishieldError(1403)
        Not a valid sort field.
    FortishieldError(1402)
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise FortishieldError(1403, extra_remediation='Allowed sort fields: {0}. '
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise FortishieldError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise FortishieldError(1724, "{}".format(', '.join(select)))
...
def delete_fortishield_file(full_path: str) -> bool:
    ...
    Raises
    ------
    FortishieldError(1906)
        File does not exist.
    FortishieldError(1907)
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.FORTISHIELD_PATH) or '..' in full_path:
        raise FortishieldError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise FortishieldError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_fortishield_limits_unchanged(new_conf, original_conf):
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise FortishieldError(1127, extra_message=f"global > limits > {disabled_limit}")
    ...
    if not data:
        with open(xml_path) as f:
            try:
                ...
                raise FortishieldError(1113, extra_message=str(e))
...
class FortishieldVersion:

    def __init__(self, version):

        pattern = r"(?:Fortishield )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise FortishieldError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise FortishieldError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class FortishieldDBBackend(AbstractDatabaseBackend):
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise FortishieldError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return FortishieldDBConnection(request_slice=self.request_slice)
    ...
    def _substitute_params(self, query, request):
        """
        Substitute request parameters in query. This is only necessary when the backend is fdb. Sqlite substitutes
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through FortishieldDB socket."""
...
class FortishieldDBQuery(object):
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        """Fortishield DB Query constructor.
        ...
        backend : str
            Database engine to use. Possible options are 'fdb' and 'sqlite3'.
        """
        self.offset = offset
        self.limit = limit
        self.table = table
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, FortishieldDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise FortishieldError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise FortishieldError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not set_select_fields or not set_select_fields.issubset(set_fields_keys):
                raise FortishieldError(1724, "Allowed select fields: {0}. Fields {1}". \
    ...
    def _parse_query(self):
        ...
        if not self.query_regex.match(self.q):
            raise FortishieldError(1407, self.q)

        level = 0
        for open_level, field, operator, value, close_level, separator in self.query_regex.findall(self.q):
            if field not in self.fields.keys():
                raise FortishieldError(1408, "Available fields: {}. Field: {}".format(', '.join(self.fields), field))
            if operator not in self.query_operators:
                raise FortishieldError(1409,
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise FortishieldError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'FortishieldDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'FortishieldDBQueryGroups':
            resource = 'name'
        else:
            raise FortishieldInternalError(1123)
...
class FortishieldDBQueryDistinct(FortishieldDBQuery):
    ...
    def _add_filters_to_query(self):
        FortishieldDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise FortishieldError(1410)

        FortishieldDBQuery._add_select_to_query(self)
...
def validate_fortishield_xml(content: str, config_file: bool = False):
    ...
    try:
        ...
        raise FortishieldError(1113, str(e))
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    Raises
    ------
    FortishieldInternalError(1005)
        Error reading file.
    FortishieldInternalError(1016)
        Error moving file.
    FortishieldError(1006)
    ...
    try:
        ...
        raise FortishieldInternalError(1005) from exc
    ...
    try:
        ...
        raise FortishieldInternalError(1016) from exc

    return results.FortishieldResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.FORTISHIELD_PATH) -> str:
    """Return a relative path from the Fortishield base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.FORTISHIELD_PATH`

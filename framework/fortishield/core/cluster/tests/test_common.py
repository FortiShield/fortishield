...
with patch('fortishield.common.fortishield_uid'):
    with patch('fortishield.common.fortishield_gid'):
        ...
        from fortishield.core.fdb import AsyncFortishieldDBConnection
...
def test_handler_msg_build_ko():
    ...
    with pytest.raises(exception.FortishieldClusterError, match=r'.* 3024 .*'):
...
async def test_handler_send_request_ko():
    ...
    with pytest.raises(exception.FortishieldClusterError, match=r'.* 3018 .*'):
        await handler.send_request(b'some bytes', b'some data')
...
async def test_handler_get_chunks_in_task_id(send_request_mock):
    ...
    # Test KeyError
    with pytest.raises(exception.FortishieldClusterError, match='.* 3035 .*'):
    ...
    with pytest.raises(exception.FortishieldClusterError, match='.* 3036 .*'):
...
@pytest.mark.asyncio
@patch('fortishield.core.cluster.common.Handler.send_request')
async def test_handler_update_chunks_fdb(send_request_mock):
    """Test that the received chunks are sent correctly to fdb."""
        ...
        with patch.object(LoggerMock, "debug") as logger_debug_mock:
            with patch.object(LoggerMock, "debug2") as logger_debug2_mock:
                with patch.object(LoggerMock, "error") as logger_error_mock:
                    assert await handler.update_chunks_fdb(
                    ...
                    logger_error_mock.assert_has_calls([call('other1'), call('other2'),
                                                        ...
                                                        call('Fortishield-db response for chunk 2/5 was not "ok": 1')])

    # Test Exception
    send_request_mock.reset_mock()
    with pytest.raises(exception.FortishieldClusterError,
                       match=r'.*Error 3037 - Error while processing Agent-info chunks: .*'):
        await handler.update_chunks_fdb(data={'chunks': [0, 1, 2, 3, 4]}, info_type='info',
    ...
    send_request_mock.assert_has_calls(
        [call(command=b'ERROR',
              data=b'error processing info chunks in process pool: '
                   b'Error 2005 - Could not connect to fdb socket: [Errno 2] No such file or directory')])
...
async def test_handler_send_file_ko():
    ...
    with pytest.raises(exception.FortishieldClusterError, match=r'.* 3034 .*'):
                ...
                with patch('fortishield.core.cluster.common.Handler.send_request', return_value="some value"):
                    with patch('json.dumps', return_value="some value"):
                        ...
                        logger_mock.assert_called_once_with(f"Error sending API response to local client: "
                                                            f"{exception.FortishieldException(1001)}")
...
@pytest.mark.parametrize('exception, expected_error', [
    (exception.FortishieldException(1001), exception.FortishieldException(1001)),
    (exception.FortishieldClusterError(3020), None),
    (Exception('Test'), exception.FortishieldClusterError(1000, extra_message='Test'))
])
                ...
                with patch('fortishield.core.cluster.common.Handler.send_request',
                    ...
                    if expected_error:
                        ...
                        send_request_mock.assert_called_once_with(
                            b'sendsync_err',
                            json.dumps(expected_error, cls=cluster_common.FortishieldJSONEncoder).encode()
                        )
                    else:
...
def test_handler_process_dapi_error_ko():
    ...
    with pytest.raises(exception.FortishieldClusterError, match=r".* 3025 .*"):
        with patch("asyncio.create_task", side_effect=exception.FortishieldClusterError(1001)):
            handler.server = ManagerMock()
...
def test_handler_process_error_from_peer():
    ...
    assert isinstance(handler.process_error_from_peer(b"data to decode"), exception.FortishieldClusterError)
...
async def test_handler_wait_for_file_ko(send_request_mock):
    """Check if expected exception is raised.
        ...
        Condition 2: when any other exception occurs, FortishieldClusterError 3040  is raised"""
    ...
    with pytest.raises(exception.FortishieldClusterError, match=r'.* 3039 .*'):
    ...
    with pytest.raises(exception.FortishieldClusterError, match=r".* 3040 .*"):
...
@pytest.mark.asyncio
@patch("fortishield.core.fdb.socket.socket")
async def test_sync_fortishield_db_retrieve_information(socket_mock):
    ...
    # data_retriever returns a JSON that does not follow the pivoting scheme
    with patch.object(sync_object, 'data_retriever', side_effect=exception.FortishieldException(1000)):
...
def test_send_data_to_fdb_ko(FortishieldDBConnection_mock):
    """Check if the data chunks are being properly forward to the Fortishield-db socket."""

    class MockFortishieldDBConnection:
        """Auxiliary class."""
    ...
    FortishieldDBConnection_mock.return_value = MockFortishieldDBConnection()

    result = cluster_common.send_data_to_fdb(data={'chunks': ['[{"data": ""}]'], 'payload': {}, 'set_data_command': ''},
                                             timeout=15, info_type='agent-groups')
    assert result['error_messages']['others'] == ['Timeout while processing agent-groups chunks.']

    FortishieldDBConnection_mock.return_value.exceptions += 1
    result = cluster_common.send_data_to_fdb(data={'chunks': ['1chunk', '2chunk'], 'set_data_command': ''},
                                             timeout=15)
...
async def test_sync_files_sync_ok(log_subprocess_mock, compress_files_mock, unlink_mock, relpath_mock, json_dumps_mock):
    ...
    class WorkerMock:
        ...
        async def send_request(self, command, data):
            ...
            if command == b"cmd" and data == b"" and self.count == 1:
                ...
                raise exception.FortishieldClusterError(3016, extra_message=command.decode())
    ...
    with patch.object(logging.getLogger("fortishield"), "error") as logger_mock:
        ...
        json_dumps_mock.assert_called_once_with(
            exception.FortishieldClusterError(code=3020, extra_message="cmd"),
    ...
    with patch.object(WorkerMock, "send_file", return_value=1000) as send_file_mock:
        ...
        with patch.object(logging.getLogger("fortishield"), "debug") as logger_debug_mock:
            with patch.object(logging.getLogger("fortishield"), "error") as logger_error_mock:
                ...
                relpath_mock.assert_called_once_with('files/path/', common.FORTISHIELD_PATH)
                ...
                log_subprocess_mock.assert_called()
                logger_error_mock.assert_called_once_with(
                    f"Error sending zip file: {exception.FortishieldException(3016, 'cmd_e')}")
...
async def test_sync_files_sync_ko(send_request_mock):
    ...
    with patch('os.unlink', side_effect=FileNotFoundError):
        ...
        with patch("fortishield.core.cluster.cluster.compress_files", return_value=(compressed_data, {})):
            send_request_mock.side_effect = None
            logger = logging.getLogger('fortishield')
            with patch.object(logger, "error") as logger_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)

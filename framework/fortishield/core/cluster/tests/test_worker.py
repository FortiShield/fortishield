...
async def test_worker_handler_connection_result(connection_result_mock, join_mock, mkdir_with_mode_mock, exists_mock,
    ...
    join_mock.assert_called_once_with(core_common.FORTISHIELD_PATH, "queue", "cluster", "Testing")
...
async def test_worker_handler_process_request_ko(logger_mock, event_loop):
    ...
    class ClientsMock:
        ...
        def send_request(self, command, error_msg):
            raise exception.FortishieldClusterError(1001)
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.server = ManagerMock()
    with pytest.raises(exception.FortishieldClusterError, match=r".* 1001 .*"):
...
async def test_worker_compare_agent_groups_checksums(socket_mock, event_loop):
    ...
    sync_object = cluster_common.SyncFortishielddb(manager=w_handler, logger=logger, cmd=b'syn_g_m_w',
...
async def test_worker_handler_recv_agent_groups_information(get_chunks_in_task_id_mock, update_chunks_fdb_mock,
    ...
    def reset_mock():
        list(map(lambda x: x.reset_mock(), [get_chunks_in_task_id_mock, update_chunks_fdb_mock,
                                            send_request_mock, check_agent_groups_checksums_mock]))
    ...
    update_chunks_fdb_mock.assert_called_once_with('chunks', 'agent-groups', logger, b'syn_w_g_err', 0)
    ...
    update_chunks_fdb_mock.assert_called_once_with('chunks', 'agent-groups', logger_c, b'syn_wgc_err', 0)
...
async def test_worker_handler_sync_integrity(request_permission_mock,
    ...
    with (patch('fortishield.core.cluster.worker.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as
        ...
        run_in_pool_mock.side_effect = exception.FortishieldException(1001)
        try:
            await asyncio.wait_for(worker_handler.sync_integrity(), 0.2)
        except asyncio.exceptions.TimeoutError:
            pass

        error_mock.assert_called_with(f"Error synchronizing integrity: {exception.FortishieldException(1001)}")
        json_dumps_mock.assert_called_with(exception.FortishieldException(1001), cls=cluster_common.FortishieldJSONEncoder)
        ...
        json_dumps_mock.assert_called_with(exception.FortishieldClusterError(code=1000, extra_message=str(Exception())),
...
async def test_worker_handler_sync_agent_info(SyncFortishielddb_mock, AsyncFortishieldDBConnection_mock, sleep_mock, event_loop):
    ...
    SyncFortishielddb_mock.return_value.request_permission = AsyncMock()
    ...
    SyncFortishielddb_mock.assert_called_once_with(manager=w_handler, logger=logger, cmd=b'syn_a_w_m', data_retriever=ANY,
                                             get_data_command='global sync-agent-info-get ',
                                             set_data_command='global sync-agent-info-set')
    SyncFortishielddb_mock.return_value.request_permission.assert_called_once()
    ...
    with patch("fortishield.core.cluster.worker.WorkerHandler.send_request") as send_request_mock:
        with patch.object(logging.getLogger("fortishield.Integrity sync"), "error") as logger_error_mock:
            ...
            logger_error_mock.assert_called_once_with(
                f"Error synchronizing extra valid files: {exception.FortishieldException(1001)}")
            send_request_mock.assert_called_once_with(command=b'syn_i_w_m_r',
                                                      data=b'None ' + json.dumps(exception.FortishieldException(1001),
                                                                                 cls=cls).encode())
...
async def test_worker_handler_process_files_from_master_ok(update_files_mock, send_request_mock, logger_debug_mock,
    ...
    with patch('fortishield.core.cluster.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as run_in_pool_mock:
        ...
        # Test first except
        worker_handler.sync_tasks["task_id"] = TaskMock()
        decompress_files_mock.side_effect = exception.FortishieldException(1001)
        event = asyncio.Event()
        await asyncio.gather(worker_handler.process_files_from_master(name="task_id", file_received=event),
                             unlock_event(event))
        ...
        json_dumps_mock.assert_called_once_with(exception.FortishieldException(1001), cls=cluster_common.FortishieldJSONEncoder)
        ...
        logger_debug_mock.assert_not_called()
        logger_info_mock.assert_called_once_with("Starting.")
        json_dumps_mock.assert_called_once_with(exception.FortishieldClusterError(code=1000, extra_message=str(Exception())),
                                                cls=cluster_common.FortishieldJSONEncoder)
        rmtree_mock.assert_not_called()
    ...
    with patch("fortishield.core.cluster.cluster.unmerge_info", return_value=[("name", "content", "_")]):
        with patch("os.remove") as os_remove_mock:
            ...
            path_join_mock.assert_has_calls([call(core_common.FORTISHIELD_PATH, 'filename1'),
                                             call(core_common.FORTISHIELD_PATH, 'name'),

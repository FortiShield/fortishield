...
def test_ac_connection_result():
    ...
    with patch.object(logging.getLogger('fortishield'), "error") as logger_mock:
        ...
        with patch.object(abstract_client.transport, "close") as close_mock:
            ...
            future.set_result([FortishieldClusterError(3020)])
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f"Could not connect to master: {str(FortishieldClusterError(3020))}.")
            ...
            future = asyncio.Future()
            future.set_exception(FortishieldClusterError(3020))
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f"Could not connect to master: {str(FortishieldClusterError(3020))}.")
            close_mock.assert_called_once()

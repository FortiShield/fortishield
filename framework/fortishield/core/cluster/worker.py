...
class WorkerHandler(client.AbstractClient, c_common.FortishieldCommon):
    """
    ...
    def connection_result(self, future_result):
        ...
        if self.connected:
            ...
            worker_tmp_files = os.path.join(common.FORTISHIELD_PATH, 'queue', 'cluster', self.name)
    ...
    def get_manager(self):
        """Get the Worker object that created this WorkerHandler. Used in the class FortishieldCommon.
    ...
    def error_receiving_integrity(self, data: str) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : str
            Task ID and error formatted as FortishieldJSONEncoder.
    ...
    async def recv_agent_groups_information(self, task_id: bytes, info_type: str, logger: logging.Logger,
        ...
        result = await super().update_chunks_fdb(data, info_type, logger, error_command, timeout)
        ...
        while True:
            ...
            # If exception is raised during sync process, notify the master so it removes the file if received.
                ...
                if isinstance(e, exception.FortishieldException):
                    exc = json.dumps(e, cls=c_common.FortishieldJSONEncoder)
                else:
                    exc = json.dumps(exception.FortishieldClusterError(1000, extra_message=str(e)),
    ...
    async def sync_agent_info(self):
        ...
        logger = self.task_loggers["Agent-info sync"]
        fdb_conn = AsyncFortishieldDBConnection()
        agent_info = c_common.SyncFortishielddb(manager=self, logger=logger, cmd=b'syn_a_w_m',
                                          data_retriever=fdb_conn.run_fdb_command,
    ...
    async def sync_extra_valid(self, extra_valid: Dict):
        ...
        # If exception is raised during sync process, notify the master, so it removes the file if received.
            ...
            if isinstance(e, exception.FortishieldException):
                ...
                exc = json.dumps(exception.FortishieldClusterError(1000, extra_message=str(e)), cls=c_common.FortishieldJSONEncoder)
            with contextlib.suppress(Exception):
                await self.send_request(command=b'syn_i_w_m_r', data=f"None {exc}".encode())
    ...
    async def process_files_from_master(self, name: str, file_received: asyncio.Event):
        ...
        if isinstance(received_filename, Exception):
            exc_info = json.dumps(exception.FortishieldClusterError(
                1000, extra_message=str(self.sync_tasks[name].filename)), cls=c_common.FortishieldJSONEncoder)
            with contextlib.suppress(Exception):
    ...
    def update_master_files_in_worker(ko_files: Dict, zip_path: str, cluster_items: Dict):
        ...
        def overwrite_or_create_files(filename_: str, data_: Dict):
            ...
            """
            full_filename_path = os.path.join(common.FORTISHIELD_PATH, filename)
            ...
            # Remove local files marked as extra.
                ...
                for file_to_remove in files:
                    try:
                        ...
                        file_path = os.path.join(common.FORTISHIELD_PATH, file_to_remove)
                        try:
                            os.remove(file_path)
                        except OSError as e:
        ...
        for directory in directories_to_check:
            try:
                full_path = os.path.join(common.FORTISHIELD_PATH, directory)

...
class MasterHandler(server.AbstractServerHandler, c_common.FortishieldCommon):
    ...
    async def execute(self, command: bytes, data: bytes, wait_for_complete: bool = False) -> Dict:
        ...
        if command == b'dapi_fwd':
            ...
            if client in self.server.clients:
                ...
                raise exception.FortishieldClusterError(3022, extra_message=client)
        ...
        if command == b'dapi' or command == b'dapi_fwd':
            try:
                ...
                raise exception.FortishieldClusterError(3021)
        # Otherwise, immediately return the result obtained before.
        else:
            status, request_result = result
            if status != b'ok':
                raise exception.FortishieldClusterError(3022, extra_message=request_result.decode())
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if self.cluster_name != self.server.configuration['name']:
            raise exception.FortishieldClusterError(3030)
        elif self.version != metadata.__version__:
            raise exception.FortishieldClusterError(3031)

        # Create directory where zips and other files coming from or going to the worker will be managed.
        worker_dir = os.path.join(common.FORTISHIELD_PATH, 'queue', 'cluster', self.name)
        ...
        # SyncFortishielddb instance to send agent-groups data to the worker.
        fdb_conn = AsyncFortishieldDBConnection()
        self.agent_groups = c_common.SyncFortishielddb(manager=self, logger=self.task_loggers['Agent-groups send'],
                                                 cmd=b'syn_g_m_w', data_retriever=fdb_conn.run_fdb_command,
    ...
    def get_manager(self) -> server.AbstractServer:
        """Get the Master object that created this MasterHandler. Used in the class FortishieldCommon.
    ...
    def process_dapi_res(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if req_id in self.server.pending_api_requests:
            ...
            raise exception.FortishieldClusterError(3032, extra_message=req_id)
    ...
    async def sync_fortishield_db_info(self, task_id: bytes, info_type: str):
        ...
        result = await self.update_chunks_fdb(data, 'agent-info', logger, b'syn_m_a_err',
    ...
    async def send_entire_agent_groups_information(self):
        ...
        sync_object = c_common.SyncFortishielddb(manager=self, logger=logger, cmd=b'syn_g_m_w_c',
                                           data_retriever=AsyncFortishieldDBConnection().run_fdb_command,
    ...
    async def sync_worker_files(self, task_id: str, received_file: asyncio.Event, logger):
        ...
        try:
            ...
            raise exception.FortishieldClusterError(3038, extra_message=str(e))
        finally:
            shutil.rmtree(decompressed_files_path)
                ...
                for file_path, data in files_metadata.items():
                    ...
                    if data['merged']:
                        for unmerged_file_path, file_data, file_time in cluster.unmerge_info(
                            ...
                            try:
                                # Destination path.
                                full_unmerged_name = os.path.join(common.FORTISHIELD_PATH, unmerged_file_path)
                                # Path where to create the file before moving it to the destination path.
                                tmp_unmerged_path = os.path.join(common.FORTISHIELD_PATH, 'queue', 'cluster', worker_name,
...
class Master(server.AbstractServer):
    ...
    async def agent_groups_update(self):
        ...
        fdb_conn = AsyncFortishieldDBConnection()
        sync_object = c_common.SyncFortishielddb(manager=self, logger=logger, cmd=b'syn_g_m_w',

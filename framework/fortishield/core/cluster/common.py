...
class Handler(asyncio.Protocol):
    ...
    def msg_build(self, command: bytes, counter: int, data: bytes) -> List[bytearray]:
        ...
        if cmd_len > self.cmd_len - len(InBuffer.divide_flag):
            raise exception.FortishieldClusterError(3024, extra_message=command)
        ...
        while parsed:
            if self.in_msg.received == self.in_msg.total:
                ...
                try:
                    ...
                    raise exception.FortishieldClusterError(3025)
    ...
    async def send_request(self, command: bytes, data: bytes) -> Union[exception.FortishieldClusterError, Any]:
        ...
        try:
            ...
            raise exception.FortishieldClusterError(3026)
        except Exception as e:
            raise exception.FortishieldClusterError(3018, extra_message=str(e))
        try:
            ...
            raise exception.FortishieldClusterError(3020, extra_message=command.decode())
    ...
    async def get_chunks_in_task_id(self, task_id: bytes, error_command: bytes) -> dict:
        ...
        try:
            ...
            raise exception.FortishieldClusterError(3035, extra_message=f"it should be under task_id {str(e)}, "
                                                                  f"but it's empty.")
        except ValueError as e:
            with contextlib.suppress(Exception):
                await self.send_request(command=error_command,
                                        data=f'error while trying to load JSON: {str(e)}'.encode())
            raise exception.FortishieldClusterError(3036, extra_message=str(e))

        return data

    async def update_chunks_fdb(self, data: dict, info_type: str, logger: logging.Logger, error_command: bytes,
                                timeout: int) -> dict:
        ...
        """
        try:
            result = await cluster.run_in_pool(self.loop, self.server.task_pool, send_data_to_fdb, data,
            ...
            raise exception.FortishieldClusterError(3037, extra_message=str(e))
    ...
    async def send_file(self, filename: str, task_id: bytes = None) -> int:
        ...
        if not os.path.exists(filename):
            raise exception.FortishieldClusterError(3034, extra_message=filename)
    ...
    async def send_string(self, my_str: bytes) -> bytes:
        ...
        try:
            ...
            with contextlib.suppress(exception.FortishieldClusterError):
    ...
    async def forward_dapi_response(self, data: bytes):
        ...
        try:
            ...
            if isinstance(e, exception.FortishieldException):
                exc = json.dumps(e, cls=FortishieldJSONEncoder)
            else:
                exc = json.dumps(exception.FortishieldClusterError(1000, extra_message=str(e)), cls=FortishieldJSONEncoder)
            with contextlib.suppress(Exception):
    ...
    def dispatch(self, command: bytes, counter: int, payload: bytes) -> None:
        ...
        try:
            ...
            command, payload = b'err', json.dumps(e, cls=FortishieldJSONEncoder).encode()
        except Exception as e:
            self.logger.error(f"Unhandled error processing request '{command}': {e}", exc_info=True)
            command, payload = b'err', json.dumps(exception.FortishieldInternalError(1000, extra_message=str(e)),
                                                  cls=FortishieldJSONEncoder).encode()
    ...
    def process_dapi_error(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if dapi_client.decode() in self.server.local_server.clients:
            ...
            except exception.FortishieldClusterError:
                raise exception.FortishieldClusterError(3025)
        else:
            raise exception.FortishieldClusterError(3032, extra_message=dapi_client.decode())
    ...
    def receive_file(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        bytes
            Response message.
        """
        self.in_file[data] = {'fd': open(common.FORTISHIELD_PATH + data.decode(), 'wb'), 'checksum': hashlib.sha256()}
    ...
    def cancel_task(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            String containing task_id and FortishieldJSONEncoded object with the exception details.
    ...
    async def wait_for_file(self, file, task_id):
        ...
        try:
            ...
            if isinstance(e, asyncio.TimeoutError):
                exc = exception.FortishieldClusterError(3039)
            else:
                exc = exception.FortishieldClusterError(3040, extra_message=str(e))
            # Notify the sending node to stop its task.
            with contextlib.suppress(Exception):
                await self.send_request(command=b"cancel_task",
                                        data=f"{task_id} {json.dumps(exc, cls=FortishieldJSONEncoder)}".encode())
            raise exc
...
class FortishieldCommon:
    """
    Task implementing common methods for both clients and servers that are Fortishield specific.
    """
    ...
    def end_receiving_file(self, task_and_file_names: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        if task_id not in self.sync_tasks:
            ...
            if os.path.exists(os.path.join(common.FORTISHIELD_PATH, filename)):
                try:
                    os.remove(os.path.join(common.FORTISHIELD_PATH, filename))
                except Exception as e:
                    self.get_logger(logger_tag).error(
                        f"Attempt to delete file {os.path.join(common.FORTISHIELD_PATH, filename)} failed: {e}")
            raise exception.FortishieldClusterError(3027, extra_message=task_id)

        # Set full path to file for task 'task_id' and notify it is ready to be read, so the lock is released.
        self.sync_tasks[task_id].filename = os.path.join(common.FORTISHIELD_PATH, filename)
    ...
    def error_receiving_file(self, task_id_and_error_details: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        task_id_and_error_details : str
             FortishieldJSONEncoded object with the exception details.
        logger_tag : str
            Logger task to use. If empty, it will use main class logger.
    ...
    async def sync(self, files: Iterable, files_metadata: Dict, metadata_len: int, task_pool=None,
        ...
        try:
            ...
            await self.server.send_request(
                ...
                data=task_id + b' ' + os.path.relpath(compressed_data, common.FORTISHIELD_PATH).encode()
            )
        except Exception as e:
            self.logger.error(f"Error sending zip file: {e}")
            if isinstance(e, exception.FortishieldException):
                exc = json.dumps(e, cls=FortishieldJSONEncoder).encode()
            else:
                exc = json.dumps(exception.FortishieldClusterError(1000, extra_message=str(e)), cls=FortishieldJSONEncoder).encode()
            with contextlib.suppress(Exception):
    ...
    async def sync(self, start_time: float, chunks: List):
        ...
        if chunks:
            ...
            if task_id.startswith(b'Error'):
                raise exception.FortishieldClusterError(3016, extra_message=f'String with agents information could '
...
def send_data_to_fdb(data, timeout, info_type='agent-info'):
    """Send chunks of data to Fortishield-db socket.
            ...
            for i, chunk in enumerate(data['chunks']):
                try:
                    if info_type == 'agent-info':
                        fdb_conn.send(f"{data['set_data_command']} {chunk}", raw=True)
...
class FortishieldJSONEncoder(json.JSONEncoder):
    """
    Define special JSON encoder for Fortishield.
    """
...
def as_fortishield_object(dct: Dict):
    try:
        ...
        raise exception.FortishieldInternalError(1000,
                                           extra_message=f"Fortishield object cannot be decoded from JSON {dct}",
                                           cmd_error=True)

...
class FortishieldDBQueryRootcheck(FortishieldDBQuery):
    """Rootcheck FortishieldDBQuery object."""
    fields = {
    ...
    def __init__(self, agent_id: str, offset: int, limit: int, sort: dict, search: dict, select: list, query: str,
        ...
        backend = FortishieldDBBackend(agent_id)
        FortishieldDBQuery.__init__(self, offset=offset, limit=limit, table='pm_event', sort=sort, search=search,
    ...
    def _filter_status(self, filter_status: dict):
        ...
        if filter_status['value'] == 'all':
            ...
            raise FortishieldException(1603, filter_status['value'])
...
def last_scan(agent_id: str) -> dict:
    ...
    # end time
    result = fdb_conn.execute(f"agent {agent_id} sql SELECT max(date_last) FROM pm_event WHERE "
    ...
    # start time
    result = fdb_conn.execute(f"agent {agent_id} sql SELECT max(date_last) FROM pm_event "
...
def rootcheck_delete_agent(agent: str, fdb_conn: FortishieldDBConnection) -> None:
    """Send message to fdb to delete the rootcheck DB of an agent.
    ...
    agent : str
        Agent ID.
    fdb_conn : FortishieldDBConnection
        Object representing the connection to the fdb socket.
    """
    fdb_conn.execute(f"agent {agent} rootcheck delete", delete=True)

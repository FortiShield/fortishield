...
class FortishieldDBQuerySyscheck(FortishieldDBQuery):
    ...
    def __init__(self, agent_id, nested=False, default_sort_field='mtime', min_select_fields=None, *args,
        ...
        super().__init__(backend=FortishieldDBBackend(agent_id), default_sort_field=default_sort_field,
...
def syscheck_delete_agent(agent: str, fdb_conn: FortishieldDBConnection) -> None:
    fdb_conn.execute(f"agent {agent} sql delete from fim_entry", delete=True)

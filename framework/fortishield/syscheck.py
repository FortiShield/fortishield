...
def run(agent_list: Union[str, None] = None) -> AffectedItemsFortishieldResult:
    ...
    """
    result = AffectedItemsFortishieldResult(all_msg='Syscheck scan was restarted on returned agents',
    ...
    with FortishieldDBQueryAgents(limit=None, select=["id", "status"], query='status!=active', **rbac_filters) as db_query:
...
def clear(agent_list: list = None) -> AffectedItemsFortishieldResult:
    ...
    AffectedItemsFortishieldResult
        Confirmation/Error message.
    """
    result = AffectedItemsFortishieldResult(all_msg='Syscheck database was cleared on returned agents',
    ...
    list(map(lambda ag: result.add_failed_item(id_=ag, error=FortishieldResourceNotFound(1701)), not_found_agents))

    fdb_conn = None
    rbac_filters = get_rbac_filters(system_resources=system_agents, permitted_resources=agent_list)
    db_query = FortishieldDBQueryAgents(select=["id", "version"], **rbac_filters)
    data = db_query.run()
    ...
    for item in data['items']:
        ...
        if agent_version is not None:
            if FortishieldVersion(agent_version) < FortishieldVersion('v3.12.0'):
                try:
                    if fdb_conn is None:
                        fdb_conn = FortishieldDBConnection()
                    syscheck_delete_agent(agent_id, fdb_conn)
                    result.affected_items.append(agent_id)
                ...
                result.add_failed_item(id_=agent_id,
                                       error=FortishieldError(1760, extra_message="Agent version should be < v3.12.0."))
        else:
            result.add_failed_item(id_=agent_id, error=FortishieldError(1015))

    if fdb_conn is not None:
        fdb_conn.close()
...
def last_scan(agent_list: list) -> AffectedItemsFortishieldResult:
    ...
    result = AffectedItemsFortishieldResult(all_msg='Last syscheck scan of the agent was returned',
    ...
    with FortishieldDBQuerySyscheck(agent_id=agent_list[0], query='module=fim', offset=0, sort=None,
...
def files(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
          ...
          summary: bool = False, distinct: bool = False) -> AffectedItemsFortishieldResult:
    ...
    result = AffectedItemsFortishieldResult(all_msg='FIM findings of the agent were returned',
    ...
    with FortishieldDBQuerySyscheck(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,

...
class FortishieldDeamonProtocolMock:
    """Auxiliary mock."""
...
def test_main(input_mock, argparse_mock, fortishield_logtest_class_mock, init_logger_mock, register_mock, logger_warning_mock,
    ...
    logger_info_mock.assert_has_calls([call('%s', 'Fortishield ERROR - KhulnaSoft Ltd.'),
    ...
    logger_error_mock.assert_called_once_with('** Fortishield-logtest error ')
...
def test_fortishield_get_description(get_version_mock):
    ...
    assert create_fortishield_class().get_description() == f"Fortishield {get_version_mock.return_value} - KhulnaSoft Ltd."

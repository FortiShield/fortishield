...
@deprecate_endpoint(link=f'https://documentation.khulnasoft.com/{__version__}/user-manual/api/reference.html#'
...
async def deprecated_login_user(user: str, raw: bool = False) -> web.Response:
    ...
    return web.Response(text=token, content_type='text/plain', status=200) if raw \
        else web.json_response(data=FortishieldResult({'data': TokenResponseModel(token=token)}), status=200, dumps=dumps)
...
async def get_user_me_policies(request, pretty: bool = False, wait_for_complete: bool = False) -> web.Response:
    ...
    """
    data = FortishieldResult({'data': request['token_info']['rbac_policies'],
...
async def revoke_all_tokens(request, pretty: bool = False) -> web.Response:
    ...
    data = raise_if_exc(await dapi.distribute_function())
    if type(data) == AffectedItemsFortishieldResult and len(data.affected_items) == 0:
        raise_if_exc(FortishieldPermissionError(4000, data.message))
